...
if($status_form==false){
 ...
 while($result_form_item = $stmt_form->fetch(PDO::FETCH_ASSOC)){
   ...
   if($result_form_item["form_type"] == "textarea"){
    $form_id_element = 'form_'.h($result_form_item["form_order"]);
    $form_item_view .= '<div class="row">';
    $form_item_view .= '<div class="col-xs-11">';
    $form_item_view .= '<div class="form-group" id="'.h($form_id_element).'">';
    $form_item_view .= '<label class="control-label" for="questions['.h($form_id_element).']">質問:</label>';
    $form_item_view .= '<textarea class="form-control" name="questions['.h($form_id_element).']" placeholder="質問文を入力してください。　　例）なぜ弊社の求人に興味をもっていただいたのでしょうか">'.h($result_form_item["question"]).'</textarea>';
    $form_item_view .= '<label class="control-label" for="answer['.h($form_id_element).'][]">回答欄:</label>';
    $form_item_view .= '<textarea class="form-control" name="answer['.h($form_id_element).'][]" disabled></textarea>';
    $form_item_view .= '<input type=hidden name="form_types['.h($form_id_element).']" value="textarea">';
    ...
    $form_item_view .= '<div class="form-group" id="'.h($form_id_element).'">';
    $form_item_view .= '<label class="control-label" for="questions['.h($form_id_element).']">質問:</label>';
    $form_item_view .= '<textarea class="form-control" name="questions['.h($form_id_element).']" placeholder="質問文を入力してください。　　例）あなたの得意な科目はなんですか？複数選択可">'.h($result_form_item["question"]).'</textarea>';
      ...
      if($status_select_item==false){
         ...
         while( $result_select_item = $stmt_select_item->fetch(PDO::FETCH_ASSOC)){
           if($result_select_item["select_item_label"]){
             $form_item_view .= '<label class="checkbox-inline"><input type="checkbox" name="answer['.h($form_id_element).'][]" disabled><input type="text" name="select_items['.h($form_id_element).'][]" value="'.h($result_select_item["select_item_label"]).'"> <span class="remove_item"><i class="glyphicon glyphicon-remove-circle"></i></span></label>';
           }
         }
       }
    $form_item_view .= '</div>';//checkbox_area
    $form_item_view .= '<input type=hidden name="form_types['.h($form_id_element).']" value="checkbox">';
    $form_item_view .= '</div>';//form-group
    $form_item_view .= '</div>';//col-xs-11
    $form_item_view .= '<div class="col-xs-1 text-right">';
    $form_item_view .= '<span class="remove"><i class="glyphicon glyphicon-trash"></i></span>';
    ...
    $form_item_view .= '<div class="form-group" id="'.h($form_id_element).'">';
    $form_item_view .= '<label class="control-label" for="questions['.h($form_id_element).']">質問:</label>';
    $form_item_view .= '<textarea class="form-control" name="questions['.h($form_id_element).']" placeholder="質問文を入力してください。　　例）あなたの血液型はなんですか？">'.h($result_form_item["question"]).'</textarea>';
    $form_item_view .= '<div class="text-right">';
    ...
    $form_item_view .= '<div class="radio_area">';
      ...
      if($status_select_item==false){//radio のアイテムを出力
         ...
         while( $result_select_item = $stmt_select_item->fetch(PDO::FETCH_ASSOC)){
           if($result_select_item["select_item_label"]){
             $form_item_view .= '<label class="radio-inline"><input type="radio" name="answer['.h($form_id_element).'][]" disabled><input type="text" name="select_items['.h($form_id_element).'][]" value="'.h($result_select_item["select_item_label"]).'"> <span class="remove_item"><i class="glyphicon glyphicon-remove-circle"></i></span></label>';
           }
         }
       }
    $form_item_view .= '</div>';//radio_area
    $form_item_view .= '<input type=hidden name="form_types['.h($form_id_element).']" value="radio">';
    $form_item_view .= '</div>';//form-group
    $form_item_view .= '</div>';//col-xs-11
    $form_item_view .= '<div class="col-xs-1 text-right">';
    $form_item_view .= '<span class="remove"><i class="glyphicon glyphicon-trash"></i></span>';
...
<div class="container">
  ...
  <div class="row">
    <div class="col-sm-1"></div>
    <div class="col-sm-8">
      <form class="form-horizontal" method="post" action="update.php?form_id=<?= h($form_id); ?>">
        <div id="myform">
          <div class="row">
            <div class="col-xs-11">
              <div class="form-group">
                <label class="control-label" for="form_name">フォーム名</label>
                <input class="form-control" type="text" name="form_name" value="<?=h($res_form_title["form_name"]);?>">
              </div>
            </div>
          ...
          <div class="row">
            <div class=col-xs-11>
              <div class="form-group">
                ...
                <textarea class="form-control" name="form_description" placeholder="フォームの説明を入力"><?= h($res_form_title["form_description"]); ?></textarea>
              </div>
            </div>
            <div class="col-xs-1"></div>
          </div>
