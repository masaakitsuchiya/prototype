...
$interview_type_str = interview_type($_SESSION["interview_type_num"]);//選考ステップ
...
if($status==false){
  ...
  //Selectデータの数だけ自動でループしてくれる
    $res = $stmt->fetch();
    $view_interviewer_name .= $res["interviewer_name"];
...
$status = $stmt->execute();
  if($status==false){
    ...
    while( $result = $stmt->fetch(PDO::FETCH_ASSOC)){
    $target_time = $result["interview_reserve_time"];
    $before_after_30minute = before_after_30minute($target_time);//該当の時間の３０分前後の時間を取得
    array_push($reserve_time_list,$result["interview_reserve_time"],$before_after_30minute[0],$before_after_30minute[1]);
    }
  }
}
...
<h3 class="text-center">通常面接予約2</h3>
...
<div class="container">
  <div class="row">
    ...
    <div class="col-sm-10">
      <form class="form-group form-horizontal" action="non_video_interview_setting_03.php" method="post">
        ...
        <div class="form-group">
          ...
          <input type="hidden" name="interview_type_num" value="<?= h($interview_type_num); ?>">
          <!-- <textarea name="json_interviewer_id" style="display:none;"><?= $json_interviewer_id ?></textarea> -->
        <div class="form-group">
        ...
        <div class="text-center">
          <a class="btn btn-default" href="non_video_interview_setting_01.php?target_interviewee_id=<?php echo($_SESSION["interviewee_id"]);?>&interview_type_num=1">戻る</a>
          &emsp;
