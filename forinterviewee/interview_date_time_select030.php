...
if($status==false){
  ...
  while( $result = $stmt->fetch(PDO::FETCH_ASSOC)){
    $interview_reserve_time_remove_second = remove_second($result["interview_reserve_time"]);

    $view .='<div class="radio text-center">';

    //interview_reserve_timeを日付と時間に分割
    $interview_reserve_date_and_time_array = explode(" ",$result["interview_reserve_time"]);//時間を削除

    $key = in_array($result["interview_reserve_time"],$not_available_time);
    if($tomorrow < $interview_reserve_date_and_time_array[0] && !$key){
      //interview_reserve_timeのほうがおおきければ、選択可能
      //そうでなければ選択不可
      $view .='<label><input type="radio" name="interview_reserve_time" id="'.h($interview_reserve_time_remove_second).'" value="'.h($result["interview_reserve_time"]).' required">'.h($interview_reserve_time_remove_second).'</label>';
    }else{
      $view .='<label class="unselectable"><input type="radio" name="interview_reserve_time" id="'.h($interview_reserve_time_remove_second).'" value="'.h($result["interview_reserve_time"]).' required" disabled="disabled">'.h($interview_reserve_time_remove_second).'</label>';
    }
    $view .='</div>';
  }
}
